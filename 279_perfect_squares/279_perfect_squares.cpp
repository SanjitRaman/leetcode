...
public:
    ...
    // due to the recursion inside the for loop.

    // instead calculate dp[i] as
    // for each i from 1...n
    //      dp[i] = min(dp[i], dp[i-j*i])
    // for all j that are square numbers less than i
    // i.e. update the minimum number of perfect square numbers
    // that sum to i if removing a larger square number improves on the result.

    int numSquares(int n) {
        std::vector<int> dp(n + 1, n); // at most n coins.
        dp[0] = 0;
        for(int i = 1; i <= n; ++i) {
            for(int j = 1; j * j <= i; ++j) {
